...
from k_space_reconstruction.nets.att_unet import AttUNet
...
class AttUnetDCModule(BaseReconstructionModule):

    def __init__(self, **kwargs):
        super(AttUnetDCModule, self).__init__(**kwargs)

    def get_net(self, **kwargs):
        return AttUnetCascade()
...
class UnetCascade(torch.nn.Module):
 ...
 #       self.cascade = torch.nn.ModuleList([Unet(1, 1, n_filters, num_layers), DataConsistencyLLearnableModule()])
        self.cascade = torch.nn.ModuleList([Unet(1, 1, n_filters, num_layers)])
...
class AttUnetCascade(torch.nn.Module):

    def __init__(self):
        super().__init__()
        self.cascade = torch.nn.ModuleList([AttUNet(1, 1), DataConsistencyLLearnableModule()])
        #self.cascade = torch.nn.ModuleList([Unet(1, 1, n_filters, num_layers)])
