...
class DDWTForward(DWTForward):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        # Prepare the filters
        filts = lowlevel.prep_filt_afb2d(h0_col, h1_col, h0_row, h1_row)
        self.h0_col = nn.Parameter(data=filts[0].repeat(J,1,1,1,1), requires_grad=True)
        self.h1_col = nn.Parameter(data=filts[1].repeat(J,1,1,1,1), requires_grad=True)
        self.h0_row = nn.Parameter(data=filts[2].repeat(J,1,1,1,1), requires_grad=True)
        self.h1_row = nn.Parameter(data=filts[3].repeat(J,1,1,1,1), requires_grad=True)
        self.coeff = nn.Parameter(data=torch.ones(J).float(), requires_grad=True)
        self.J = J
        self.mode = mode
    ...
    def forward(self, x):
        ...
        mode = lowlevel.mode_to_int(self.mode)
        # Do a multilevel transform
        for i, j in enumerate(range(self.J)):
            # Do 1 level of the transform
            ll, high = lowlevel.AFB2D.apply(
                ll, self.h0_col[i], self.h1_col[i], self.h0_row[i], self.h1_row[i], mode)
            yh.append(high)
            res.append(ll.norm(p=1))
        res = torch.stack(res)
        res = self.coeff / res
        return res.sum(), ll, yh
